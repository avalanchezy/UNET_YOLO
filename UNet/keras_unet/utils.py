...
def ReadMasks(images_files, size, crop=None):
    ...
    for index in tqdm(range(len(images_files))):
        ...
        image_read = np.where(image_read > 0, 1, 0)  # Assuming any non-zero value is a lesion
        y.append(image_read.astype(np.int16))
    y = np.asarray(y, dtype=np.int16)
    y = np.expand_dims(y, -1)  # Adding the channel dimension for compatibility with Keras
    return y